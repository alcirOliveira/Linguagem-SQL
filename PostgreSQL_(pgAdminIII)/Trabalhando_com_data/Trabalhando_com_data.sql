...
SELECT
   nome,
   FLOOR((CURRENT_DATE - dt_nasc) / 365) AS idade
FROM funcionario 
WHERE (CURRENT_DATE - dt_nasc) / 365 > 30;

-- Selecionando o nome, data de admissão e tempo de casa dos funcionários que trabalham na empresa a mais de 5 anos
SELECT
   nome,
   dt_adm,
   FLOOR((CURRENT_DATE - dt_adm) / 365) AS tempo_casa
FROM funcionario
WHERE (CURRENT_DATE - dt_adm) / 365 > 5;

-- Selecionando o nome, data de admissão e tempo de casa dos funcionários que trabalham na empresa a menos de 10 anos
SELECT
   nome,
   dt_adm,
   FLOOR((CURRENT_DATE - dt_adm) / 365) AS tempo_casa
FROM funcionario
WHERE (CURRENT_DATE - dt_adm) / 365 < 10;
